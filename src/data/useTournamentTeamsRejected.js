...
export default function useTournamentTeamsRejected () {
  ...
  const fetchTeams = async function (tournamentId) {
    ...
    try {
      const result = await tournamentsService.fetchTournamentTeamsRejected({ tournamentId })
      if (isStale()) { return; }
      ...
      console.error('Error fetching rejected teams for tournament', { tournamentId }, e)
      setError(e.message)
    }
  }
